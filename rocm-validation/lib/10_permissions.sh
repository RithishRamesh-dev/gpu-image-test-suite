...
test_permissions() {
  ...
  if [[ -c /dev/kfd ]]; then
    ...
    # Current user in render/video group?
    local current_groups
    current_groups=$(groups 2>/dev/null || id -Gn 2>/dev/null || echo "")
    if echo "$current_groups" | grep -qE "render|video|kfd"; then
      record_pass "user_in_gpu_group" "User in: $(echo "$current_groups" | tr ' ' '\n' | grep -E 'render|video|kfd' | tr '\n' ' ')"
    else
      record_warn "user_in_gpu_group" "User not in render/video/kfd group — container access may fail"
    fi
  else
  ...
  # /dev/dri permissions
  local dri_files
  dri_files=$(ls /dev/dri/ 2>/dev/null | wc -l)
  if [[ "$dri_files" -gt 0 ]]; then
    local dri_perms
    dri_perms=$(stat -c '%a' /dev/dri/renderD128 2>/dev/null || stat -c '%a' /dev/dri/card0 2>/dev/null || echo "unknown")
  ...
  # udev rules for ROCm
  if find /etc/udev/rules.d /lib/udev/rules.d 2>/dev/null | grep -q "rocm\|amdgpu\|kfd"; then
    record_pass "udev_rules_present" "ROCm udev rules found"
  else
    record_warn "udev_rules_present" "No ROCm udev rules found — device access may require root"
  fi

  # Check for world-writable ROCm binaries (security)
  local world_writable
  world_writable=$(find /opt/rocm/bin 2>/dev/null -perm -o+w | head -5 | wc -l)
  ...
  # Setuid binaries in ROCm
  local setuid_count
  setuid_count=$(find /opt/rocm 2>/dev/null -perm /4000 | wc -l)
