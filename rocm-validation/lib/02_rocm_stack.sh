...
test_rocm_stack() {
  ...
  # rocminfo shared library linkage
  local broken_libs
  broken_libs=$(ldd "$(which rocminfo)" 2>/dev/null | grep -c "not found" || echo "0")
  if [[ "$broken_libs" -eq 0 ]]; then
  ...
  # Capture rocminfo output
  capture "rocminfo" rocminfo

  # Version detection from rocminfo
  local detected_ver
  ...
  if [[ -z "$detected_ver" ]] && cmd_exists amd-smi; then
    detected_ver=$(amd-smi version 2>/dev/null | grep -oP "ROCm:\s*\K[0-9]+\.[0-9]+\.[0-9]+" || true)
  fi

  if [[ "$detected_ver" == "$ROCM_EXPECTED_VERSION" ]]; then
    record_pass "rocm_version" "Detected $detected_ver == expected $ROCM_EXPECTED_VERSION"
  ...
  if cmd_exists hipcc; then
    ...
    # HIP version should match ROCm major.minor
  ...
  # rocminfo shows at least one Agent
  local agent_count
  agent_count=$(rocminfo 2>/dev/null | grep -c "^  Name:" || echo "0")
  ...
  # ROCm libraries present
  for lib in libhip_hcc.so libamdhip64.so librocblas.so; do
    if find /opt/rocm /usr/lib 2>/dev/null | grep -q "$lib"; then
  ...
  # No mixed ROCm version files
  local old_rocm_files
  old_rocm_files=$(find /opt/rocm 2>/dev/null -name "*.so.*" | xargs -I{} sh -c 'strings {} 2>/dev/null | grep -o "rocm-[0-9]\+\.[0-9]\+\.[0-9]\+" | head -1' 2>/dev/null | grep -v "^$" | sort -u | grep -v "$ROCM_EXPECTED_VERSION" | head -5 || true)
