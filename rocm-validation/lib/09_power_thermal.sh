...
test_power_thermal() {
  ...
  # Throttle state check
  local throttle_count
  throttle_count=$(rocm-smi --showrasinfo all 2>/dev/null | grep -ci "throttle\|thermal_throttle" || echo "0")
  ...
  [[ "$power_gpu_idx" -gt 0 ]] && info "Total GPU power draw: ${total_power}W across $power_gpu_idx GPU(s)"

  # Clock frequencies
  local clk_issues
  clk_issues=$(rocm-smi --showclocks 2>/dev/null | grep -ci "Mhz 0\b" || echo "0")
